...
\end{figure}
	...
	\begin{itemize}
		...
		\end{itemize}
		\item \textbf{DTO (Data Transfer Objects):} Classi immutabili (\texttt{Book}, \texttt{User}, \texttt{Library}), \texttt{Rating}, \texttt{Suggestion}) usate per il trasporto dati. Implementano \texttt{Serializable} per la trasmissione RMI.
		\item \textbf{"Extended" DTO:} Classi immutabili (\texttt{BookInfo}, \texttt{SuggestionWithBooks}, \texttt{SuggestionCount}, \texttt{AverageRatings}), usati per trasporto dati, ma sono contenitori di DTO.
...
\end{figure}
	...
	\begin{enumerate}
		...
		\item \textbf{Massimo consigliati:} Un utente può consigliare al massimo tre libri riferiti ad un altro libro.
		\item \textbf{Limite Voti:} I voti appartententi ad una valutazione libro devono essere per forza nel range 1-5.
		\item \textbf{Limite Note:} Le note per una valutazione libro devono essere al massimo di 256 caratteri.
	\end{enumerate}
	...
	\begin{figure}[htbp]
		...
		\includegraphics[width=0.8\textwidth, keepaspectratio]{er_diagram_4.pdf}
	...
	\begin{itemize}
		\item \textbf{Associazione "Contiene" (Molti-a-Molti):} L'associazione tra \textit{Librerie} e \textit{Libri} presenta cardinalità N:N. Nel modello relazionale, questo vincolo strutturale impone la creazione di una tabella ponte ("bridge table") denominata \textbf{LibriXLibrerie}. Essa contiene le chiavi esterne verso le due entità collegate; la chiave primaria della tabella è costituita dalla chiave composta (\textit{libreria\_id}, \textit{libro\_id}).
		
		\item \textbf{Associazione "Valuta" con attributi:} Sebbene concettualmente definita come associazione tra \textit{Utente} e \textit{Libro}, questa associazione possiede numerosi attributi propri (punteggi parziali e note testuali). Pertanto, è stata tradotta nella tabella \textbf{ValutazioniLibri}.
		
		\item \textbf{Associazione "Consiglia" (Ternaria):} Questa associazione coinvolge tre entità logiche: l'Utente (autore del consiglio), il Libro Sorgente e il Libro Consigliato. Nello schema logico si traduce nella tabella \textbf{ConsigliLibri}, dove tre chiavi esterne garantiscono l'integrità referenziale.
	\end{itemize}
	...
	\begin{itemize}
		...
		\item \textbf{Libreria - Libro (N:N):} Una libreria può contenere un numero arbitrario di libri e uno stesso libro può essere catalogato nelle librerie di diversi utenti.
	...
	Ogni modulo Maven (\texttt{common}, \texttt{serverBR}, \texttt{clientBR}, \texttt{db\_init}) contiene un descrittore \texttt{module-info.java}.
	...
	La documentazione completa è consultabile dal file "\texttt{docs/javadocs/index.html}".
	...
	I DTO sono stati impiegati per il trasferimento dei dati tra Client e Server. Le classi \texttt{Book}, \texttt{User}, \texttt{Library}, \texttt{Rating} e \texttt{Suggestion} (nel package \texttt{common.dto}) ne sono l'esempio.
	\begin{itemize}
	...
	\begin{itemize}
		...
		\item \textbf{RPair:} È stata introdotta la classe generica \texttt{BRPair<A, B>} (implementata come \texttt{record} Java) per gestire il ritorno di valori multipli dai metodi RMI (es. Esito Operazione + Oggetto Remoto).
		\item \textbf{HashMap:} Utilizzate nel frontend per associare il testo visualizzato nelle liste con l'oggetto a cui fanno riferimento;
	\end{itemize}
	...
	\begin{itemize}
		...
		\item \textbf{Lato Client (Costruzione UI):} Una volta ricevuti i risultati, il Client deve mappare i DTO ricevuti in oggetti grafici per la \texttt{ListView} e \texttt{HashMap}. Questo processo ha un costo lineare $\mathcal{O}(R)$, dove $R$ è il numero di libri restituiti dalla ricerca.
	\end{itemize}
	...
	\section{Ottimizzazione del Calcolo della Media dei Voti}
