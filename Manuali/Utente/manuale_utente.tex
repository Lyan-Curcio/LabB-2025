...
\begin{document}
	...
	L'applicazione è destinata principalmente a:
	\begin{itemize}
		\item \textbf{Imprese:} Per la gestione delle librerie e il miglioramento della \textit{customer experience} tramite sistemi di valutazione e consigli.
		\item \textbf{Utenti privati:} Per curare la propria esperienza di lettura e consultare i pareri della community.
	\end{itemize}
	...
	\begin{itemize}
		...
		\item \textbf{PostgreSQL:} Necessario per il funzionamento del database (gestito tramite gli script di inizializzazione).
	...
	\begin{enumerate}
		\item Scaricare e decomprimere la cartella del progetto in una posizione locale (es. \filename{Documenti/BookRecommender}).
		\item Assicurarsi che all'interno siano presenti i file di avvio (\filename{.bat} per Windows o \filename{.sh} per Linux/Mac).
	\end{enumerate}
	...
	\begin{enumerate}
		\item Aprire il terminale e spostarsi nella cartella del progetto:
		\begin{verbatim}
			$ cd path/to/BookRecommender
		\end{verbatim}
		\item \textbf{Primo avvio (Inizializzazione Database):}
		\begin{verbatim}
			$ ./InitDB.sh
		\end{verbatim}
		\item \textbf{Avvio Server:}
		\begin{verbatim}
			$ ./Server.sh
		\end{verbatim}
		\item \textbf{Avvio Client:} Aprire un nuovo terminale ed eseguire:
		\begin{verbatim}
			$ ./Client.sh
		\end{verbatim}
	\end{enumerate}
	...
	\begin{figure}[H]
		\centering
		\fbox{\begin{minipage}{0.8\textwidth}\centering \vspace{3cm} [Inserire Screenshot Benvenuto.fxml - Home] \vspace{3cm} \end{minipage}}
	...
	\begin{enumerate}
		\item \textbf{Selezione Filtro:} Utilizzare il menu a tendina (SplitMenuButton) per scegliere il criterio:
		\begin{itemize}
	...
	\begin{figure}[H]
		\centering
		\fbox{\begin{minipage}{0.8\textwidth}\centering \vspace{3cm} [Inserire Screenshot InformazioniLibro.fxml] \vspace{3cm} \end{minipage}}
	...
	\begin{figure}[H]
		\centering
		\fbox{\begin{minipage}{0.8\textwidth}\centering \vspace{3cm} [Inserire Screenshot Registrazione.fxml] \vspace{3cm} \end{minipage}}
	...
	\begin{itemize}
		...
		\item \textbf{Monitorare i propri dati:} La barra laterale destra mostra automaticamente:
		\begin{itemize}
	...
	\begin{figure}[H]
		\centering
		\fbox{\begin{minipage}{0.8\textwidth}\centering \vspace{3cm} [Inserire Screenshot Libreria.fxml] \vspace{3cm} \end{minipage}}
		\caption{Vista interna di una libreria personale.}
		\label{fig:libreria}
	\end{figure}
	...
	\begin{figure}[H]
		\centering
		\fbox{\begin{minipage}{0.8\textwidth}\centering \vspace{3cm} [Inserire Screenshot Valutazione.fxml] \vspace{3cm} \end{minipage}}
		\caption{Schermata di valutazione con i 5 criteri.}
	...
	\begin{itemize}
		\item \textbf{Causa:} Il database potrebbe essere vuoto.
		\item \textbf{Soluzione:} Se è il primo utilizzo, assicurarsi di aver eseguito lo script \filename{InitDB} per popolare il catalogo iniziale.
	\end{itemize}
